...
export class ProductDataService {
    ...
    // ── Static in-memory product store ────────────────────────────
    ...
    create(product: Omit<IProduct, 'id'>): IProduct {
        const newProduct: IProduct = { ...product, id: this.nextId++ } as IProduct;
        this.products.push(newProduct);
        return newProduct;
    }

    // ── UPDATE ────────────────────────────────────────────────────
    update(id: number, updated: Partial<IProduct>): IProduct | null {
    ...
    filterByCategory(category: string): IProduct[] {
        return category === '' ? this.getAll() : this.products.filter(p => p.category === category);
    }
}
